...
type ColumnType = ColumnDef<ContentInterface, unknown>;
...
export function ContentListPage(props: ContentListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ContentInterface>>(
    () => `/contents?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ContentInterface) => {
    if (hasAccess('content', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/contents/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'title', header: 'title', accessorKey: 'title' },
    { id: 'body', header: 'body', accessorKey: 'body' },
